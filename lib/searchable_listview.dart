...
class SearchFailure extends StatelessWidget {
  ...
  const SearchFailure({Key? key, this.text = 'Error'}) : super(key: key);
...
class SearchLoading extends StatelessWidget {
  ...
  const SearchLoading({Key? key, this.text = 'Loading'}) : super(key: key);
...
class SearchablePager<T> extends StatefulWidget {
  ...
  //Added by Awri properties
  final bool filterPropertiesMap = false;
  //
  ...
  SearchablePager({
    ...
    this.filter,
    this.loadingWidget,
    this.errorWidget = const SearchFailure(), 
    this.searchTextController,
    this.keyboardAction = TextInputAction.done,
    this.inputDecoration,
...
class _SearchablePagerState<T> extends State<SearchablePager<T>> {
  ...
  List<Map<String, dynamic>> filterMapList(List<Map<String, dynamic>> list, String query) {
    final keys = list.isNotEmpty ? list.first.keys.toList() : [];

    return list.where((element) {
          final coincident = keys.firstWhere((filterKey) {
            if (element[filterKey] != null) {
              return element[filterKey].toString().toLowerCase().contains(query.toLowerCase());
            }
            return false;
          },
          orElse: () => null,);
  ...
  InputDecoration baseInputDecoration({required String placeholder, Color? background, Color? border}) {
